...
failures << "approved hero artwork is missing" unless File.file?(hero_asset) && html.include?('src="assets/brand/hero-sunrise.png"')
failures << "hero artwork intrinsic dimensions are missing" unless html.include?('width="1536" height="1024"')
failures << "temporary hero placeholder remains" if html.match?(/Production asset pending|data-required-asset|art-placement/)
hero_rule = css[/\.hero-art-placeholder \{([^}]*)\}/, 1].to_s
failures << "hero artwork remains inside a styled frame" if hero_rule.match?(/padding|border|background|box-shadow|border-radius|overflow/)
failures << "hero background does not match the artwork's deep navy edge" unless css.include?("background: #000310;")
