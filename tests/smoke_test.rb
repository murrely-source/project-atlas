...
failures << "shared public assets are not loaded" unless html.include?('href="site.css?v=0.22.9"') && html.include?('src="brand-config.js?v=0.22.7"') && html.include?('src="contact-config.js"') && html.include?('src="site.js?v=0.22.8"')
favicon_references = [
...
approved_solutions_narrative_copy = [
...
failures << "approved Solutions narrative is incomplete or altered" unless approved_solutions_narrative_copy.all? { |copy| html.include?(copy) }
solutions_section = html[/<section class="section" id="solutions".*?<\/section>/m].to_s
...
failures << "contact submit listener is not registered" unless script.include?('function initializeContactForm()') && script.include?('contactForm.addEventListener("submit", async (event) =>') && script.include?("initializeContactForm();")
failures << "Turnstile initialization is not connected" unless script.include?("window.onSolarisTurnstileReady = initializeContactTurnstile") && script.match?(/if \(window\.turnstile\) \{\s*initializeContactTurnstile\(\);\s*\}/)
failures << "configured contact endpoint is not used" unless script.include?("fetch(contactConfig.endpointUrl")
failures << "contact Sending state is not entered and restored" unless script.include?("setContactSubmitting(true)") && script.match?(/finally \{\s*setContactSubmitting\(false\);\s*\}/)
