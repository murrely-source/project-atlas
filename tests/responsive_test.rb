...
failures << "resource grid does not collapse on mobile" unless css.match?(/@media \(max-width: 767px\).*?\.resource-grid \{ grid-template-columns: 1fr; \}/m)
...
failures << "hero heading and action groups are incomplete" unless html.match?(/<div class="page-container hero-layout">\s*<div class="hero-heading">.*?<\/div>\s*<div class="hero-support">\s*<div class="button-group">/m)
failures << "hero actions are not confined to their responsive region" unless css.match?(/\.hero-support \{[^}]*width: min\(100%, 32rem\);/)
failures << "removed Hero supporting sentence remains" if html.include?("Guiding organizations through AI adoption") || html.include?('class="hero-intro"') || css.include?(".hero-intro") || config.include?("positioning:")
