...
failures << "desktop three-column overview is missing" unless css.include?("grid-template-columns: repeat(3,minmax(0,1fr))")
failures << "tablet two-column overview is missing" unless css.match?(/@media\(max-width:1100px\).*?\.overview-grid \{ grid-template-columns: repeat\(2,1fr\); \}/m)
failures << "mobile one-column grids are missing" unless css.match?(/@media\(max-width:767px\).*?\.grid, \.overview-grid \{ grid-template-columns: 1fr; \}/m)
...
failures << "all tables must have accessible contained scrolling" unless table_count == 7 && scroll_count == table_count
...
failures << "Return to News Desk touch target is undersized" unless css.match?(/\.drawer-return \{[^}]*min-height: 44px;/)
...
failures << "Solaris Labs corporate lockup lacks responsive sizing" unless css.match?(/@media\(max-width:767px\).*?\.brand-name \{ font-size: 24px; \}.*?\.brand-subline \{ font-size: 7px; \}/m) && css.match?(/@media\(max-width:480px\).*?\.brand-name \{ font-size: 20px;.*?\.brand-subline \{[^}]*font-size: 6px;/m)
failures << "theme-specific Solaris Labs assets do not share responsive dimensions" unless html.include?('class="brand-symbol brand-symbol-dark"') && html.include?('class="brand-symbol brand-symbol-light"') && css.include?(".brand-symbol { width: 76px; height: 76px; object-fit: contain; }")
failures << "Light-theme optical alignment is missing or changes layout" unless css.include?(':root[data-theme="light"] .brand-symbol-light { display: block; transform: translateY(-4px); }') && !css.match?(/\.brand-symbol-light[^}]*margin|\.brand-symbol-light[^}]*position:/)
failures << "Dark-theme optical alignment is missing or changes layout" unless css.include?(':root[data-theme="dark"] .brand-symbol-dark { transform: translateY(-4px); }') && !css.match?(/\.brand-symbol-dark[^}]*margin|\.brand-symbol-dark[^}]*position:/)
failures << "Standards & Learning desktop filter layout is missing" unless css.include?(".standards-filter-grid { display: grid; grid-template-columns: repeat(5,minmax(0,1fr));")
failures << "Standards & Learning responsive layouts are missing" unless css.match?(/@media\(max-width:1100px\).*?\.standards-filter-grid \{ grid-template-columns: repeat\(2,minmax\(0,1fr\)\); \}/m) && css.match?(/@media\(max-width:767px\).*?\.standards-filter-grid, \.standards-section-grid \{ grid-template-columns: 1fr; \}/m)
failures << "Standards & Learning filters are not touch friendly" unless css.match?(/\.standards-filter-grid select \{[^}]*min-height: 44px;/)
...
puts "  desktop 3-column, tablet 2-column, and mobile 1-column overview grids validated"
